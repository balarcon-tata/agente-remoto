...
function freshMemory(){return{line:null,date:null,order:null,style:null,operation:null,metric:null,target:null,lastPlan:null,lastSelection:null};}
...
function fuzzy(text,items,strip=false,minScore=.55){const n=norm(text);let best=null,score=0;for(const raw of items){let l=norm(raw);if(strip)l=l.replace(/^\d+\s+/,'');const parts=l.split(' ').filter(x=>x.length>2);let s=n.includes(l)?1:(parts.length?parts.filter(p=>n.includes(p)).length/parts.length:0);if(parts.some(p=>p.length>5&&n.includes(p)))s=Math.max(s,.7);if(s>score){score=s;best=raw}}return score>=minScore?best:null}
...
function explicit(text,rows){
  ...
  const operation=fuzzy(text,c.operations,false,.72),lr=resolveLine(text,rows);
...
function semantic(text,raw,memory,rows){
  ...
  // La linea activa se conserva por defecto en un seguimiento implícito.
  if(!e.line&&!globalLineQ&&memory.line)p.filters.line=memory.line;
  // Un dia activo tambien se conserva, excepto si el usuario pide semana/tendencia o cambia linea sin dia.
  ...
  const wantsOpList=/\b(listado|lista|desglose|desglosa|detalle|muestrame|muéstrame|quiero ver)\b.*\boperacion|\boperaciones\b.*\b(listado|lista|desglose|detalle)|\bpor operacion|\bpor operación|\boperaciones de|\bque operaciones|\bqué operaciones/.test(n);
  if(wantsOpList){
  ...
  if(/\bpor dia|por día|cada dia|cada día\b/.test(n)&&!wantsOpList&&!wantsOrderList){p.groupBy='date';p.action='group';p.target='day';p.output='chart';}
...
function filterRows(rows,f={}){return rows.filter(r=>(!f.line||norm(r.linea)===norm(f.line))&&(!f.date||r.fecha===f.date)&&(!f.order||norm(r.orden)===norm(f.order))&&(!f.style||String(r.estilo)===String(f.style)))}
...
function plannerPrompt(q,memory,rows){return `Eres el PLANIFICADOR de un agente especialista en el programa de Ensamble. NO respondas. Devuelve SOLO JSON valido. Datos: fecha,linea/familia,orden,estilo,unidades,hc,operaciones[{operacion,sam,eficiencia,hc}]. Lineas: ${catalogs(rows).lines.join(' | ')}. Memoria: ${JSON.stringify(memory)}. Pregunta: ${q}. Reglas: demanda/volumen/cantidad/programa=units; personal/gente/HC/recursos=hc; carga/cargado sin unidades explicitas=hc; operacion critica=mayor hc; operacion compleja/dificil=mayor sam; familia=linea; referencias ese/esa usan memoria; desglose de operaciones=group operation con hc; que dia esta una orden=detail order date; por que/analiza=reasons. No inventes lineas: usa solo las Lineas listadas. Schema: {"action":"aggregate|rank|group|detail|count|compare|analyze|explain|relationship|clarify","target":"area|line|order|style|operation|day","metric":"units|hc|sam|efficiency|count|date|multi","groupBy":"none|date|line|order|style|operation","criterion":"none|max|min","filters":{"line":null,"date":null,"order":null,"style":null,"operation":null},"output":"brief|table|chart|analysis"}`}
