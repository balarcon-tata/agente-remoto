...
function freshMemory(){return{area:null,line:null,date:null,order:null,style:null,operation:null,metric:null,target:null,groupBy:null,lastPlan:null,lastSelection:null,scope:{area:null,line:null,date:null,order:null,style:null,operation:null,metric:null},focus:{entity:null,value:null,scope:null},conversationState:{lastResult:null,lastUserIntent:null,turn:0}};}
...
function resolveLine(text,rows){
  ...
  // Una pregunta que PIDE la línea ("¿en qué línea está esa operación?") no
  // contiene una línea candidata. No debemos convertir el resto de la frase en
  // un nombre de línea inválido.
  const asksWhichLine=/\b(?:en\s+)?(?:que|cual)\s+linea\b|\blinea\s+(?:esta|corresponde|pertenece)\b/.test(n);
  if(asksWhichLine)return{line:null,invalid:null};
...
function semantic(text,raw,memory,rows){
  ...
  const followUpQ=/^(y\s+|ahora\s+|entonces\s+|en ese caso\s+)?(dame|muestra|muestrame|muéstrame|quiero ver|despliega|despliegue|detalle|desglose|como|cómo|cuanto|cuánto|cuanta|cuánta|cuantas|cuántas|por dia|por día|solo|de eso|de esa|de ese|que pasa|qué pasa|y si|si ahora)\b/.test(n)||wantsByDay||/\b(esa operacion|esa operación|dicha operacion|dicha operación|lo anterior|mismo alcance|ese mismo|ese alcance|ahi|ahí)\b/.test(n);
  ...
  const lastSel=memory.lastSelection||{};
  const refOrderValue=memory.order||(lastSel.entity==='order'?lastSel.value:null);
  const refStyleValue=memory.style||(lastSel.entity==='style'?lastSel.value:null);
  const refOpValue=memory.operation||(lastSel.entity==='operation'?lastSel.value:null);
  const refDayValue=memory.date||(lastSel.entity==='day'?lastSel.value:null);
  if(refOrder&&refOrderValue)p.filters.order=refOrderValue;
  if(refStyle&&refStyleValue)p.filters.style=refStyleValue;
  if(refOp&&refOpValue)p.filters.operation=refOpValue;
  if(refDay&&refDayValue)p.filters.date=refDayValue;
...
function ambiguity(text,p,memory,rows){
  ...
  const activeRefDay=memory.date||(lastSel.entity==='day'?lastSel.value:null);
  if(refDay&&!activeRefDay){
  ...
  const activeRefOrder=memory.order||(lastSel.entity==='order'?lastSel.value:null);
  if(refOrder&&!activeRefOrder){
...
// V42: las filas universales pueden repetirse por operación. Las unidades son demanda del
// programa y se cuentan una sola vez por fecha+línea+orden+estilo; el HC sí se suma por operación.
function productionKey(r){return [String(r.fecha||'').slice(0,10),norm(r.linea),norm(r.orden),String(r.estilo||'')].join('|')}
function uniqueUnits(rows){const m=new Map();for(const r of rows||[]){const k=productionKey(r),u=num(r.unidades);if(!m.has(k)||u>m.get(k))m.set(k,u)}return [...m.values()].reduce((s,u)=>s+u,0)}
function flatOps(rows){return rows.flatMap(r=>(r.operaciones||[]).map(o=>({key:o.operacion,hc:num(o.hc),sam:num(o.sam),efficiency:num(o.eficiencia),units:num(r.unidades),date:r.fecha,line:r.linea,order:r.orden,style:String(r.estilo),productionKey:productionKey(r)})))}
function agg(rows,entity){const key={line:'linea',order:'orden',style:'estilo',day:'fecha'}[entity],m=new Map();for(const r of rows){const k=String(r[key]??'—'),x=m.get(k)||{key:k,units:0,hc:0,count:0,unitKeys:new Map(),dates:new Set(),lines:new Set(),styles:new Set(),orders:new Set()};const pk=productionKey(r),u=num(r.unidades);if(!x.unitKeys.has(pk)||u>x.unitKeys.get(pk))x.unitKeys.set(pk,u);x.hc+=num(r.hc);x.count++;x.dates.add(r.fecha);x.lines.add(r.linea);x.styles.add(String(r.estilo));x.orders.add(r.orden);m.set(k,x)}return[...m.values()].map(x=>({...x,units:[...x.unitKeys.values()].reduce((s,u)=>s+u,0),unitKeys:undefined,dates:[...x.dates],lines:[...x.lines],styles:[...x.styles],orders:[...x.orders]}))}
function aggOps(rows){const m=new Map();for(const o of flatOps(rows)){const x=m.get(o.key)||{key:o.key,hc:0,units:0,sam:0,effNum:0,effDen:0,unitKeys:new Map(),orders:new Set(),styles:new Set(),dates:new Set(),lines:new Set()};x.hc+=o.hc;if(!x.unitKeys.has(o.productionKey)||o.units>x.unitKeys.get(o.productionKey))x.unitKeys.set(o.productionKey,o.units);x.sam=Math.max(x.sam,o.sam);const w=Math.max(o.hc,.0001);x.effNum+=o.efficiency*w;x.effDen+=w;x.orders.add(o.order);x.styles.add(o.style);x.dates.add(o.date);x.lines.add(o.line);m.set(o.key,x)}return[...m.values()].map(x=>({...x,units:[...x.unitKeys.values()].reduce((s,u)=>s+u,0),unitKeys:undefined,efficiency:x.effDen?x.effNum/x.effDen:0,orders:[...x.orders],styles:[...x.styles],dates:[...x.dates],lines:[...x.lines]}))}
...
if(p.action==='detail'&&p.target==='order'){const order=p.filters.order||memory.order,rr=rows.filter(x=>norm(x.orden)===norm(order));if(!rr.length){r.ok=false;r.error='No encontré esa orden.';return r}r.entity='order';r.selected={order,line:rr[0].linea,date:rr[0].fecha,style:String(rr[0].estilo)};r.data={order,dates:uniq(rr.map(x=>x.fecha)),lines:uniq(rr.map(x=>x.linea)),styles:uniq(rr.map(x=>String(x.estilo))),units:uniqueUnits(rr),hc:rr.reduce((s,x)=>s+num(x.hc),0),operations:aggOps(rr).sort((a,b)=>b.hc-a.hc)};return r}
...
if(p.action==='aggregate'){r.entity=p.target;r.data={units:uniqueUnits(rs),hc:rs.reduce((s,x)=>s+num(x.hc),0),orders:uniq(rs.map(x=>x.orden)).length,styles:uniq(rs.map(x=>String(x.estilo))).length,lines:uniq(rs.map(x=>x.linea)).length};return r}
...
if(['analyze','explain','compare'].includes(p.action)){const byLine=agg(rs,'line').sort((a,b)=>b.hc-a.hc),byOrderU=agg(rs,'order').sort((a,b)=>b.units-a.units),byOrderH=agg(rs,'order').sort((a,b)=>b.hc-a.hc),byStyle=agg(rs,'style').sort((a,b)=>b.units-a.units),ops=aggOps(rs),opH=[...ops].sort((a,b)=>b.hc-a.hc),opS=[...ops].sort((a,b)=>b.sam-a.sam),days=agg(rs,'day').sort((a,b)=>a.key.localeCompare(b.key));r.data={units:uniqueUnits(rs),hc:rs.reduce((s,x)=>s+num(x.hc),0),lineCount:uniq(rs.map(x=>x.linea)).length,orderCount:uniq(rs.map(x=>x.orden)).length,styleCount:uniq(rs.map(x=>String(x.estilo))).length,topLines:byLine.slice(0,5),topOrderUnits:byOrderU[0],topOrderHC:byOrderH[0],topStyleUnits:byStyle[0],topOperationHC:opH[0],topOperationSAM:opS[0],days};return r}
...
function update(memory,p,r){
  ...
  // A ranked/selected answer is not automatically a new conversational scope.
  // Keep it separately so explicit references such as "esa orden" still work.
  if(r?.entity&&r?.selected){
    m.lastSelection={
      entity:r.entity,
      value:r.selected?.[r.entity]||r.selected?.key||r.selected?.date||null,
      data:{...r.selected},
      scope:{
        area:m.area||null,
        line:m.line||null,
        date:m.date||null,
        order:r.selected?.order||r.selected?.key||(r.entity==='order'?(r.selected?.[r.entity]||null):null),
        style:r.selected?.style||(r.entity==='style'?(r.selected?.[r.entity]||r.selected?.key||null):null),
        operation:r.selected?.operation||(r.entity==='operation'?(r.selected?.[r.entity]||r.selected?.key||null):null)
      }
    };
    m.focus={
      entity:r.entity,
      value:m.lastSelection.value,
      scope:{...(m.lastSelection.scope||{})}
    };
  }else if(r?.entity&&p.action!=='rank'){
    m.lastSelection={entity:r.entity,value:null,data:null,scope:null};
  }
...
return{DAYS,norm,uniq,num,fmt,dayName,mentionedDates,freshMemory,catalogs,resolveLine,explicit,sanitize,semantic,ambiguity,filterRows,productionKey,uniqueUnits,flatOps,agg,aggOps,value,execute,update,plannerPrompt};
});
